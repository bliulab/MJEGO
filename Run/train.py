...
# ==== Init ====
ckp_dir = "/data0/wzw/PFP-Pred-Proj/Log/{}/{}/".format(args.ont, "model_test_33")
...
def train(start_epoch, min_val_loss, model, optimizer, criterion, data_loader):
    ...
    # === Save Result ===
    result_dir = f"/data0/wzw/PFP-Pred-Proj/Results/{args.ont}"
    os.makedirs(result_dir, exist_ok=True)
    np.savez_compressed(
        os.path.join(result_dir, "33_all_epochs_val_results.npz"),
        **all_epoch_results
    )
...
wandb.init(project="Protein_Function_Prediction", config=config,
           name="{}_{}".format("IDEA222_train", args.ont))
